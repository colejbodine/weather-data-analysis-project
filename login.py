...
class LoginGUI:
    ...
    def log_in(self):
        ...
        if any(user['username'] == username.lower() and user['password'] == password for user in user_accounts):
            tk.messagebox.showinfo("Log In", f"Logging in as {username}. Nothing for now, but stay tuned for "
                                             f"Milestone #2!")
        ...
        elif any(user['username'] == username.lower() and not user['password'] == password for user in user_accounts):
